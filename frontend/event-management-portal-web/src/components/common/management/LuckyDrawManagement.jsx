...
const LuckyDrawManagement = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      resetForm();
    } catch (err) { alert("Lỗi lưu dữ liệu"); } finally { setIsSubmitting(false); }
  };
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        {showCreateButton && (
          <button
            ...
            className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-lg text-sm font-medium transition-all shadow-md"
          >
                  ...
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 text-center">
                      <div className="flex justify-center gap-2">
                        ...
                        <button onClick={() => handleDeleteClick(item.id)} className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg"><Trash2 size={16} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
                {luckyDraws.length === 0 && !loading && (
                  <tr>
                    <td colSpan="5" className="p-10 text-center text-slate-400">Không có dữ liệu vòng quay nào</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden border-4 border-white flex flex-col max-h-[90vh]">
              ...
              <div className="p-8 border-t bg-slate-50/80 flex justify-end gap-3">
                ...
                <button disabled={isSubmitting} onClick={handleSubmit} className="flex items-center gap-3 px-10 py-4 bg-indigo-600 text-white font-black rounded-2xl shadow-xl hover:bg-indigo-700 uppercase text-[11px] tracking-widest transition-all">
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-slate-500 text-sm mb-6">
                Bạn có chắc chắn muốn xóa vòng quay này không? Hành động này không thể hoàn tác.
              </p>
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-2.5 rounded-xl bg-red-600 text-white font-semibold hover:bg-red-700 transition-colors shadow-lg shadow-red-200 disabled:bg-red-400"
                >
                  {isSubmitting ? <Loader2 className="animate-spin mx-auto" size={20} /> : "Xóa ngay"}
                </button>
              </div>
            </motion.div>
          </div>
