...
import {
  ...
  Save, X, ArrowLeft, Lock, Eye, EyeOff, CheckCircle, AlertCircle
} from "lucide-react";
...
const UserProfile = () => {
  ...
  const handleChangePassword = async () => {
    ...
    if (!passwordForm.confirmPassword) errs.confirmPassword = "Vui lòng xác nhận mật khẩu";
    else if (passwordForm.newPassword !== passwordForm.confirmPassword) errs.confirmPassword = "Mật khẩu xác nhận không khớp";
    ...
    try {
      const token = localStorage.getItem("accessToken");
      await axios.patch(
        `${import.meta.env.VITE_AUTH_API_URL}/profiles/me/change-password`,
        { oldPassword: passwordForm.oldPassword, newPassword: passwordForm.newPassword, confirmPassword: passwordForm.confirmPassword },
        { headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" } }
      );
      ...
      <div className="max-w-5xl mx-auto px-4 -mt-28">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-[300px] shrink-0">
            <div className="bg-white rounded-3xl shadow-[0_4px_24px_rgba(0,0,0,0.08)] p-6 text-center border border-slate-100 sticky top-6">
              ...
              <p className="text-xs text-slate-400 mb-4 truncate">{accountInfo.email}</p>
              ...
              <div className="mt-4 pt-4 border-t border-slate-100">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-red-500 hover:bg-red-50 transition-all cursor-pointer"
                >
          ...
          <div className="flex-1">
            <div className="bg-white rounded-3xl shadow-[0_4px_24px_rgba(0,0,0,0.08)] border border-slate-100 p-8 min-h-[520px]">
              ...
              {activeTab === "info" ? (
                <div>
                  <div className="flex items-center justify-between mb-7 pb-4 border-b border-slate-100">
                    ...
                    {!isEditing ? (
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => { setIsEditing(false); setFormData({ fullName: profile?.fullName || "", dateOfBirth: profile?.dateOfBirth || "", gender: profile?.gender || "", majorName: profile?.majorName || "" }); }} className="flex items-center gap-2 px-4 py-2 bg-slate-100 text-slate-600 rounded-xl font-semibold text-sm hover:bg-slate-200 transition-all cursor-pointer">
                          <X size={14} /> Hủy
                        </button>
                      </div>
                    )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <InfoBox label="Mã định danh" value={profile?.id ? `#${profile.id.substring(0, 8).toUpperCase()}` : "N/A"} icon={CheckCircle2} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="max-w-md space-y-5">
                    <PasswordField
                      ...
                      onChange={(e) => { setPasswordForm({ ...passwordForm, oldPassword: e.target.value }); setPasswordErrors({ ...passwordErrors, oldPassword: "" }); }}
                    ...
                    <div className="bg-blue-50 rounded-xl p-4 border border-blue-100">
                      <p className="text-xs font-bold text-blue-700 mb-2">Yêu cầu mật khẩu:</p>
                      {[
                        { check: passwordForm.newPassword.length >= 8, text: "Ít nhất 8 ký tự" },
                        { check: /[A-Z]/.test(passwordForm.newPassword), text: "Ít nhất 1 chữ hoa" },
                        { check: /[a-z]/.test(passwordForm.newPassword), text: "Ít nhất 1 chữ thường" },
                        { check: /[!@#$%^&*]/.test(passwordForm.newPassword), text: "Ít nhất 1 ký tự đặc biệt" },
                      ].map(({ check, text }) => (
                        <div key={text} className="flex items-center gap-2 mt-1">
                          <div className={`w-4 h-4 rounded-full flex items-center justify-center transition-all ${check ? "bg-emerald-500" : "bg-slate-200"}`}>
                            {check && <CheckCircle size={10} className="text-white" />}
                          </div>
                          <span className={`text-xs transition-colors ${check ? "text-emerald-600 font-semibold" : "text-slate-400"}`}>{text}</span>
                        </div>
                      ))}
                    </div>
                    ...
                    <button
                      ...
                      className="w-full py-3 bg-[#1a3a6b] text-white rounded-xl font-semibold text-sm hover:bg-[#15306b] active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-lg shadow-[#1a3a6b]/25 disabled:opacity-60 cursor-pointer"
                    >
                      {isChangingPassword ? (
...
const SidebarBtn = ({ active, onClick, icon, label }) => (
  <button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all cursor-pointer ${active ? "bg-[#1a3a6b] text-white shadow-lg shadow-[#1a3a6b]/25" : "text-slate-600 hover:bg-slate-50"}`}>
    {icon} {label}
  </button>
);
...
const EditableInput = ({ label, value, isEditing, onChange, icon: Icon, type = "text" }) => (
  <div className={`p-4 rounded-2xl border transition-all duration-200 ${isEditing ? "bg-white border-[#1a3a6b]/30 ring-2 ring-[#1a3a6b]/10" : "bg-slate-50/70 border-slate-100"}`}>
    ...
    {isEditing ? (
      <input type={type} value={value || ""} onChange={onChange} className="w-full text-sm font-semibold text-slate-800 bg-transparent outline-none border-b border-[#1a3a6b]/20 focus:border-[#1a3a6b] transition-colors py-0.5 cursor-text" />
    ) : (
...
const EditableSelect = ({ label, value, isEditing, onChange, icon: Icon, options }) => (
  <div className={`p-4 rounded-2xl border transition-all duration-200 ${isEditing ? "bg-white border-[#1a3a6b]/30 ring-2 ring-[#1a3a6b]/10" : "bg-slate-50/70 border-slate-100"}`}>
    ...
    {isEditing ? (
      <select value={value || ""} onChange={onChange} className="w-full text-sm font-semibold text-slate-800 bg-transparent outline-none border-b border-[#1a3a6b]/20 focus:border-[#1a3a6b] transition-colors py-0.5 cursor-pointer">
...
const PasswordField = ({ label, value, show, onToggle, onChange, error, placeholder }) => (
  <div className="space-y-1.5">
    ...
    <div className="relative">
      ...
      />
      <button type="button" onClick={onToggle} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors cursor-pointer">
