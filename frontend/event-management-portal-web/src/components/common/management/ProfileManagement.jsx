...
import {
  User, Mail, Shield, Camera, Briefcase, Calendar, CheckCircle,
  Edit2, Save, X, Phone, UserCircle, MapPin, Loader2
} from "lucide-react";
...
const ProfileManagement = ({
  ...
  const badgeColorClass = accentColor === "blue" ? "bg-blue-50 text-blue-600 border-blue-100" : "bg-emerald-50 text-emerald-600 border-emerald-100";
  const saveBtnClass = accentColor === "blue" ? "bg-blue-600 hover:bg-blue-700 shadow-blue-200" : "bg-indigo-600 hover:bg-indigo-700 shadow-indigo-200";
  ...
  return (
    ...
    >
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden mb-8 relative">
        <div className={`h-32 bg-gradient-to-r ${headerGradient}`} />
          ...
          <div className="relative -mt-16 flex flex-col md:flex-row items-center md:items-end gap-6 mb-4 text-center md:text-left">
            <div className="relative group">
              <div className="w-32 h-32 bg-white rounded-3xl p-1 shadow-xl border border-white/50 backdrop-blur-sm">
                <img
                  src={profile?.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(profile?.fullName || "User")}&background=${accentColor === 'blue' ? '1e293b' : '0284c7'}&color=fff&size=200`}
                  alt="Avatar"
                  className="w-full h-full object-cover rounded-2xl"
                />
              </div>
              <button className={`absolute -bottom-2 -right-2 p-2.5 text-white rounded-xl shadow-lg transition-all hover:scale-105 active:scale-95 group-hover:rotate-6 ${saveBtnClass}`}>
                <Camera size={18} />
              </button>
            </div>
              ...
              <div className="flex flex-col md:flex-row items-center md:items-center gap-4 justify-between">
                <div>
                  <div className="flex items-center justify-center md:justify-start gap-3 mb-1">
                    ...
                    <span className={`px-3 py-1 text-[10px] font-black rounded-full flex items-center gap-1.5 border uppercase tracking-wider ${badgeColorClass}`}>
                      {accentColor === "blue" ? <Shield size={12} /> : <CheckCircle size={12} />} {role}
                    </span>
                  </div>
                  <div className="flex flex-wrap items-center justify-center md:justify-start gap-x-4 gap-y-1 text-slate-500 text-sm font-medium">
                    <span className="flex items-center gap-1.5"><Shield size={14} className={`text-${accentColor}-500`} /> {profile?.loginCode || "Chưa cập nhật mã số"}</span>
                  ...
                  {editMode ? (
                    <>
                      ...
                      <button
                        onClick={handleSave}
                        disabled={saving}
                        className={`flex items-center gap-2 px-6 py-2.5 text-white rounded-xl font-bold text-sm transition-all shadow-lg active:scale-95 disabled:opacity-50 ${saveBtnClass}`}
                      >
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 px-6 py-2.5 bg-white border-2 border-slate-100 text-slate-800 rounded-xl font-black text-sm transition-all active:scale-95 shadow-sm hover:bg-slate-50 hover:border-${accentColor}-100 hover:text-${accentColor}-600`}
                    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-left">
        ...
        <div className="space-y-8">
          <div className="bg-slate-900 p-8 rounded-[2rem] text-white shadow-xl shadow-slate-200">
            ...
            <div className="space-y-5">
              <div className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/10">
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase mb-0.5">Trạng thái</p>
                  <p className="text-sm font-medium">Bảo mật nâng cao</p>
                </div>
                <span className={`text-[10px] font-black px-3 py-1.5 rounded-full border ${accentColor === 'blue' ? 'text-blue-400 bg-blue-400/10 border-blue-400/20' : 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20'}`}>
                  {accentColor === 'blue' ? 'ROOT' : 'ACTIVE'}
                </span>
              </div>
          ...
          <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Lịch sử hoạt động</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-500 font-medium italic">Ngày tham gia</span>
...
const EditItem = ({
  label, value, icon, isEditing, onChange, placeholder,
  type = "text", options = [], displayValue, accentColor = "indigo"
}) => {
  const focusColorClass = accentColor === "blue" ? "focus:border-blue-500" : "focus:border-indigo-500";
    ...
    <div className="flex items-start gap-4">
      ...
      <div className="flex-1">
        ...
        {isEditing ? (
          type === "select" ? (
            <select
              className={`w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-2 text-sm font-bold focus:bg-white transition-all outline-none ${focusColorClass}`}
              value={value || ""}
