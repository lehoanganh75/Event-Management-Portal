...
const Sidebar = ({ isCollapsed, onToggle }) => {
    ...
    return (
        <>
            <aside
                className={`
        relative bg-[var(--sidebar-bg)] border-r border-[var(--border-color)] h-screen sticky top-0 flex flex-col shadow-sm z-[110]
      ...
      `}
            >
                ...
                <button
                    onClick={onToggle}
                    className="absolute -right-4 top-8 w-8 h-8 bg-[var(--bg-secondary)] border-2 border-[var(--border-color)] rounded-full flex items-center justify-center text-slate-500 hover:text-indigo-600 hover:border-indigo-400 shadow-lg hover:shadow-indigo-100 transition-all z-100 cursor-pointer group/toggle"
                >
                    {isCollapsed ? (
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {!isCollapsed && (
                            <div className="flex flex-col overflow-hidden whitespace-nowrap transition-all duration-300">
                                <span className="text-sm font-black text-[var(--text-primary)] tracking-tight leading-none uppercase">Event Portal</span>
                ...
                {!isCollapsed && (
                    <div className="px-8 pt-8 pb-3">
                        <span className="text-[11px] font-bold text-slate-400 uppercase tracking-[0.12em]">{t('MENU') || "Menu chính"}</span>
                    </div>
                )}
            ...
            `}>
                  ...
                  ${isCollapsed ? 'justify-center px-0' : 'gap-3.5'}
                  ${isActive
                                            ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 font-bold border border-indigo-100/50 dark:border-indigo-500/20'
                                            : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-slate-100'}
                `}>
                                        <div className={`
                    flex items-center justify-center w-9 h-9 rounded-lg transition-all shrink-0
                    ${isActive ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 group-hover:bg-slate-200 dark:group-hover:bg-slate-700'}
                  `}>
                ...
                <div className={`p-4 border-t border-[var(--border-color)] mt-auto bg-slate-50/50 dark:bg-slate-900/50 overflow-hidden ${isCollapsed ? 'px-2' : 'px-4'}`}>
                    <NavLink to="/" className="outline-none block">
                        <div className={`
            group flex items-center rounded-xl text-sm font-bold text-slate-500 dark:text-slate-400 hover:bg-[var(--bg-secondary)] hover:text-indigo-600 transition-all border border-transparent hover:border-[var(--border-color)] hover:shadow-sm
            ${isCollapsed ? 'justify-center p-2' : 'gap-3.5 px-4 py-3'}
          `}>
                            <div className={`
              flex items-center justify-center w-9 h-9 rounded-lg bg-slate-200 dark:bg-slate-800 text-slate-500 dark:text-slate-400 group-hover:bg-indigo-600 group-hover:text-white transition-all shrink-0
            `}>
