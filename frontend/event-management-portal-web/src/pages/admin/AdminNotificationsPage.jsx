...
import {
  ...
  Calendar, User, Mail, Send, X, Loader2, PlayCircle, CheckCircle2
} from "lucide-react";
...
const AdminNotificationsPage = () => {
  ...
  const handleBulkDelete = async () => {
    ...
    if (selectedIds.length === 0) return alert("Vui lòng chọn thông báo để xóa");
  ...
  }, [notifications, searchTerm, activeTab, filterType]);
    ...
    <div className="p-6 bg-slate-50 min-h-screen text-left">
      ...
      <div className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-800 tracking-tight">QUẢN LÝ THÔNG BÁO</h1>
            <p className="text-sm text-slate-500 mt-1">Gửi và điều phối thông báo hệ thống</p>
          </div>
        </div>
      ...
      {/* STATS CARDS (Matching Events Management) */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-slate-700 text-white p-5 rounded-2xl shadow-sm">
          <div className="flex items-center gap-3">
            <Mail size={28} />
            <div>
              <p className="text-sm opacity-90">Hệ thống</p>
              <p className="text-3xl font-semibold mt-1">{stats.system}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex border-b border-slate-200 mb-6 overflow-x-auto pb-1 gap-2">
        {[
          ...
          { id: "Hệ thống", label: "Hệ thống", icon: Info, count: stats.system },
        ].map((tab) => (
          <button
            key={tab.id}
