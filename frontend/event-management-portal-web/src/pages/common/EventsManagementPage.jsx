...
const EventsManagementPage = () => {
  ...
  // Detect mode: SUPER_ADMIN is the system admin, others (including ADMIN/Lecturer) are lecturers
  const isSystemAdmin = useMemo(() => {
    const roles = user?.roles || user?.account?.roles || (user?.role ? [user.role] : []);
    return roles.some(r => {
      const roleName = typeof r === 'string' ? r : r.name;
      return roleName?.toUpperCase() === "SUPER_ADMIN";
    });
  }, [user]);
  ...
  const handleApproveEvent = async (id) => {
    try {
      ...
      showToast("Lỗi khi duyệt sự kiện", "error");
    }
  };
  ...
  const handleApprovePlan = async (id) => {
    try {
      ...
      showToast("Duyệt kế hoạch thành công");
      fetchData();
    } catch (error) {
      showToast("Lỗi khi duyệt kế hoạch", "error");
    }
  };
  ...
  const handleRejectPlan = async (id) => {
    ...
    try {
      ...
      showToast("Đã không thông qua kế hoạch");
      fetchData();
    } catch (error) {
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-semibold text-slate-800">
            {isSystemAdmin ? "Quản lý sự kiện hệ thống" : "Sự kiện của tôi"}
          </h1>
        </div>
      ...
      <div className="flex border-b border-slate-200 mb-6 overflow-x-auto pb-1 gap-2">
        {[
          { id: "Tất cả", label: "Tất cả", icon: Calendar, count: stats.total },
          { id: "Sắp tới", label: "Sắp tới", icon: Clock, count: stats.upcoming },
          { id: "Đang diễn ra", label: "Đang diễn ra", icon: PlayCircle, count: stats.ongoing },
          { id: "Hoàn thành", label: "Hoàn thành", icon: CheckCircle2, count: stats.completed },
                  ...
                  <tr key={e.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="flex justify-center gap-1.5">
                        <button
                          onClick={() => navigate(isSystemAdmin ? `/admin/events/${e.id}` : `/lecturer/events/${e.id}`)}
                        ...
                        {isSystemAdmin && (
                          <>
                            {e.status === "PLAN_PENDING_APPROVAL" && (
                              <>
                                <button
                                  onClick={() => handleApprovePlan(e.id)}
