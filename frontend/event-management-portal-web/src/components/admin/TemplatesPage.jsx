...
const TemplatesPage = () => {
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen text-left font-sans">
      ...
      {/* FILTER BAR */}
      <div className="bg-white/70 backdrop-blur-md border border-white rounded-[2rem] p-4 mb-8 flex flex-wrap gap-4 items-center shadow-xl shadow-slate-200/50">
        <div className="relative flex-1 min-w-[300px]">
          <Search className="absolute left-4 top-3.5 text-slate-400" size={18} />
          <input
            className="pl-12 pr-4 py-3.5 w-full bg-slate-50/50 border border-slate-100 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:bg-white transition-all font-medium"
        ...
        <div className="flex bg-slate-100/50 p-1 rounded-2xl gap-1">
          {["Tất cả", "Công khai", "Nội bộ"].map(tab => (
            <button
              key={tab}
      ...
      {/* TABLE */}
      <div className="bg-white/80 backdrop-blur-lg border border-white rounded-[2.5rem] shadow-2xl shadow-slate-200/40 overflow-hidden">
        {loading ? (
          <div className="py-32 flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-slate-400 font-black text-sm uppercase tracking-widest animate-pulse">Đang tải dữ liệu...</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50/50">
                <tr>
                  <th className="px-8 py-5 text-left text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Thông tin chung</th>
                  <th className="px-8 py-5 text-left text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Thiết lập mặc định</th>
                  <th className="px-8 py-5 text-left text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Hệ thống</th>
                  <th className="px-8 py-5 text-center text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Thống kê</th>
                  <th className="px-8 py-5 text-right text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Thao tác</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {currentTemplates.map((t) => (
                  <tr key={t.id} className="hover:bg-blue-50/20 group transition-all duration-300">
                    <td className="px-8 py-6">
                      <div className="flex flex-col gap-1.5">
                        <span className="font-black text-slate-800 text-lg leading-tight group-hover:text-blue-600 transition-colors">{t.templateName}</span>
                        <p className="text-xs text-slate-400 font-medium line-clamp-1 italic">"{t.description || "Không có mô tả"}"</p>
                        <div className="flex flex-wrap gap-1.5 mt-1">
                          <span className="bg-slate-800 text-white px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-wider shadow-sm">
                            {t.faculty || "CHUNG"}
                          </span>
                        </div>
                      </div>
                    ...
                    <td className="px-8 py-6">
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-slate-500 font-bold bg-slate-50 w-fit px-2 py-1 rounded-lg">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col gap-1.5">
                        {t.public ? 
                          <span className="flex items-center gap-1.5 text-emerald-600 text-[10px] font-black uppercase bg-emerald-50 w-fit px-2 py-0.5 rounded-lg">
                            <Globe size={12}/> Công khai
                          </span> : 
                          <span className="flex items-center gap-1.5 text-slate-400 text-[10px] font-black uppercase bg-slate-100 w-fit px-2 py-0.5 rounded-lg">
                            <Lock size={12}/> Nội bộ
                          </span>
                        }
                        <div className="flex gap-1">
                          <div className={`w-2.5 h-2.5 rounded-full shadow-inner ${t.configData?.certificate ? 'bg-emerald-500' : 'bg-slate-300'}`} title="Chứng chỉ" />
                          <div className={`w-2.5 h-2.5 rounded-full shadow-inner ${t.configData?.requireApproval ? 'bg-amber-500' : 'bg-blue-500'}`} title="Phê duyệt" />
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col items-center">
                        <div className="flex items-center gap-1 text-slate-800 bg-slate-100 px-3 py-1.5 rounded-xl font-black text-base shadow-inner">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex justify-end gap-2">
                        <button 
                          ...
                          className="p-2.5 bg-slate-50 text-slate-400 hover:text-blue-600 hover:bg-white hover:shadow-lg rounded-2xl transition-all duration-300 active:scale-90"
                          title="Xem chi tiết"
                        >
                          <Eye size={20} />
                        </button>
                        <button 
                          ...
                          className="p-2.5 bg-slate-50 text-slate-400 hover:text-amber-600 hover:bg-white hover:shadow-lg rounded-2xl transition-all duration-300 active:scale-90"
                          title="Chỉnh sửa"
                        >
                          <Settings size={20} />
                        </button>
                        <button 
                          ...
                          className="p-2.5 bg-slate-50 text-slate-400 hover:text-rose-600 hover:bg-white hover:shadow-lg rounded-2xl transition-all duration-300 active:scale-90"
                          title="Xóa mẫu"
                        >
                          <Trash2 size={20} />
                        </button>
          ...
          <div className="px-8 py-6 bg-slate-50/50 flex items-center justify-between border-t border-slate-100">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="p-2.5 rounded-2xl border border-white bg-white text-slate-500 hover:bg-slate-50 hover:shadow-lg disabled:opacity-30 disabled:cursor-not-allowed transition-all"
              >
                <ChevronLeft size={20} />
              </button>
              <button
            ...
            >
              ...
              <div className="px-10 py-8 flex justify-between items-center bg-slate-50 border-b border-slate-100">
                <div className="flex items-center gap-4">
                  <div className={`w-14 h-14 rounded-[1.25rem] flex items-center justify-center text-white shadow-xl ${
                    modalMode === "view" ? "bg-blue-600 shadow-blue-100" : "bg-amber-600 shadow-amber-100"
                  }`}>
